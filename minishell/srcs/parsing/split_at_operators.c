...
/*   Updated: 2021/09/14 18:16:39 by wszurkow         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int count_operators(char *cmd_str)
{
	int operator_count;
	...
	while (1)
	{
		...
		cmd_str += next_operator_index + operator_type(cmd_str) + 1;
		operator_count++;
	}
}
...
char **split_at_operators(char *cmd_str)
{
	...
	i = 0;
	cmd_tab = malloc(sizeof(char  **) * ((count_operators(cmd_str) * (2 + 1) + 1)));
	if (!(cmd_tab))
		return (NULL);
	cmd_tab[count_operators(cmd_str) * 2 + 1] = NULL;
	while (1)
	{
		next_operator_index = find_index_of_next_operator(cmd_str);
